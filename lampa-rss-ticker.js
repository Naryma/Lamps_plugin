(function () {
    ...
    // ---------- Constants ----------
    // Надійніша стрічка замість Google News (менше проблем з CORS)
    var DEFAULT_URL   = 'https://www.pravda.com.ua/rss/';
    var MAX_ITEMS     = 20;
    ...
    // Список CORS-проксі (пробуємо по черзі)
    var PROXIES = [
        ...
        try {
            ...
            Lampa.SettingsApi.addParam({
                ...
                field: {
                    ...
                    description: 'Посилання на XML/RSS/Atom. Рекомендовано: https://www.pravda.com.ua/rss/'
                },
            ...
            Lampa.SettingsApi.addParam({
                ...
                field: { name: 'Відступ знизу', description: 'Якщо стрічка перекриває нижнє меню або обрізається на ТВ' },
            ...
            Lampa.SettingsApi.addParam({
                ...
                field: {
                    ...
                    description: 'Необов\'язково. Якщо вказати — буде використовуватися тільки він. Інакше плагін сам пробує кілька публічних проксі.'
                },
                onChange: function () { later(reloadFeed); }
            });
        } catch (e) {
            console.log('RSS ticker: settings error', e);
        }
    }
    ...
    function restartAnimation() {
        if (!$content || !$wrap || $wrap.hasClass('hide-ticker')) return;
    ...
    function isPlayerOpen() {
        ...
        try {
            return !!(Lampa.Player && Lampa.Player.opened && Lampa.Player.opened());
        } catch (e) {
            return false;
        }
    }
    ...
    function buildRoutes(feedUrl) {
        ...
        if (custom) {
            return [custom + encodeURIComponent(feedUrl)];
        }

        // Спочатку пробуємо напряму, потім через проксі
        var routes = [feedUrl];
        for (var i = 0; i < PROXIES.length; i++) {
            routes.push(PROXIES[i] + encodeURIComponent(feedUrl));
        }
        return routes;
    }
    ...
    function loadRSS() {
        ...
        fetchFeed(buildRoutes(feedUrl), 0, id, function (titles) {
            ...
            if (!hasNews) {
                setText('Помилка завантаження RSS. Спробуйте інший URL або проксі.');
            }
        });
    }
