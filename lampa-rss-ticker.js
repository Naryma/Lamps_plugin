(function () {
        ...
        try {
            ...
            Lampa.SettingsApi.addParam({
                ...
                field: {
                    ...
                    description: 'Необов\'язково. Префікс, до якого додається закодований URL стрічки. Якщо порожньо: спочатку напряму, потім через allorigins.win'
                },
                onChange: function () { later(reloadFeed); }
            });
        } catch (e) {
    ...
    function loadRSS() {
        ...
        fetchFeed(buildRoutes(feedUrl), 0, id, function (titles) {
            ...
            if (!hasNews) {
                setText('Помилка завантаження RSS. Перевірте URL або проксі в налаштуваннях.');
            }
        });
    }
